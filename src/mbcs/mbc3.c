...
typedef struct rtc_t {
    ...
    uint8_t REG_0B;
    uint8_t REG_0C;
    uint8_t REG_08_LATCHED;
    uint8_t REG_09_LATCHED;
    uint8_t REG_0A_LATCHED;
    uint8_t REG_0B_LATCHED;
    uint8_t REG_0C_LATCHED;
    uint64_t timestamp;
} rtc_t;
...
rtc_t* allocRtc(){ void* out = malloc(sizeof(rtc_t)); memset(out, 0, sizeof(rtc_t)); return out; }
...
uint8_t mbc3_4000_7FFF(gb_t* gb, uint16_t addr){
    ...
    real_addr &= (1 << 14) - 1;
    uint8_t bank = mbc->REG_2000_3FFF == 0x00 ? 0x01 : (mbc->REG_2000_3FFF & 0b1111111); 
    real_addr |= bank << 14;
