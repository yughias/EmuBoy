...
bool containNintendoLogo(uint8_t* buffer){
...
bool isNewLicenseeCode(uint8_t* buffer){
    return buffer[0x14B] == 0x33;
}

const char* getRomName(uint8_t* buffer){
    return (const char*)(&buffer[0x134]);
}

const char* getManufacturerName(uint8_t* buffer){
    uint8_t idx = buffer[0x14B];
...
size_t getRomSize(uint8_t* buffer){
